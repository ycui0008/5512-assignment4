...
ggmap(usa_map) +
  geom_segment(data=day1, 
                     aes(x=Origin_lon, 
                         ...
                         yend=Dest_lat), 
                     colour="Red", size = 1) +
  geom_point(data=day1, aes(x=Origin_lon, Origin_lat),
...
p1 <- day1 %>%
  ...
  select(Origin_lon, Origin_lat, Dest_lon, Dest_lat, Dep_DateTime, AirTime_inter)
p1
```
...
```{r plot the break down point}
ggmap(usa_map) +
  geom_point(data=day1, aes(x=Origin_lon, Origin_lat),
             colour="Red", size=2) +
  geom_point(data = tibble1, aes(x = value_1,
                                 y = value_2)) +
  geom_point(data = tibble2, aes(x = value_1,
                                 y = value_2))
```
