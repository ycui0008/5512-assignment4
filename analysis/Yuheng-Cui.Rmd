...
```{r choose one airport, , eval=FALSE}
airport <- d %>%
...
load(file = here::here("data","flights.rda"))
flight_UA <-  flight%>%
  filter(Reporting_Airline == "UA")
```
...
pathtable <- function(data,i){
  test1 <- (1:(data$AirTime[i]%/%15))
  
  #longitude change sequence
  lon_incre <- ((data$Dest_lon[i] - data$Origin_lon[i])*15*test1)/(data$AirTime[i]) + data$Origin_lon[i]

  #latitude change sequence
  lat_incre <- ((data$Dest_lat[i] - data$Origin_lat[i])*15*test1)/(data$AirTime[i]) + data$Origin_lat[i]
  
  #time change
  time_incre <- data$DepTime[i] + step*test1
...
```{r animatedplot for one plane in a day}
ggmap(usa_map) +
  geom_point(data = path_1, aes(x= Origin_lon, y = Origin_lat), size = 2) +
...
```{r path_way}
path_way <- bind_rows(pathtable(e4,1),pathtable(e4,2),pathtable(e4,3),pathtable(e4,4),pathtable(e4,5),pathtable(e4,6),pathtable(e4,7),pathtable(e4,8),pathtable(e4,9),pathtable(e4,10),pathtable(e4,11),pathtable(e4,12),pathtable(e4,13),pathtable(e4,14))
```
